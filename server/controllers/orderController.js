...
const updateOrderStatus = async (req, res) => {
    try {
        ...
        if (order) {
            // Check if user is admin, supplier, or the buyer
            ...
            console.log(`Update status request for order ${req.params.id}`);
            console.log(`User: ${req.user._id} (${req.user.role})`);
            console.log(`Order Buyer: ${order.buyer}, Supplier: ${order.supplier}`);
            console.log(`IsAdmin: ${isAdmin}, IsSupplier: ${isSupplier}, IsBuyer: ${isBuyer}`);

            if (!isAdmin && !isSupplier && !isBuyer) {
                console.log('Authorization failed: User is neither Admin, Supplier, nor Buyer');
                return res.status(401).json({ message: 'Not authorized to update this order' });
            }

            // If user is ONLY the buyer, they can only set status to 'delivered'
            if (isBuyer && !isAdmin && !isSupplier) {
                if (req.body.status !== 'delivered') {
                    console.log(`Forbidden: Buyer tried to set status to ${req.body.status}`);
                    return res.status(403).json({ message: 'You can only confirm delivery' });
                }
            }
...
const checkProductPurchase = async (req, res) => {
    try {
        const order = await Order.findOne({
            buyer: req.user._id,
            'items.inventoryItem': req.params.productId
        });
